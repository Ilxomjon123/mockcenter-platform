...
export function useReadingDragAndDrop(options: DragAndDropOptions) {
  ...
  const showOptionByKey = (key: string) => {
    // Search globally since options might be in a different panel
  ...
  const saveToStorage = () => {
    // readingStore already handles storage via updateAnswer
  }
  ...
  const handleDropOnZone = (dropzone: HTMLElement) => {
    ...
    // Save to store
    readingStore.updateAnswer(parseInt(matchNumber, 10), draggedValue)

    // Handle source: either an option or another dropzone
    if (draggedOption) {
      draggedOption.classList.add('used')
      // Also mark globally in case option is in a different panel
      const globalOption = document.querySelector(
        `.draggable-option[data-option-key="${draggedValue}"]`
      ) as HTMLElement
      if (globalOption && globalOption !== draggedOption) {
        globalOption.classList.add('used')
      }
